...
# Decide the action based on the following criteria:
#   - If theta is spinning too far to the right, fire right thruster; too far left, left thruster
#   - If the velocity in absolute is greater than a threshold, fire main engine
def heuristic(state):
    x, y, vx, vy, theta, omega, legL, legR = state
    OMEGA_THRESH = 0.05
    THETA_THRESH = 0.1
    H_VELOCITY_THRESH = 0.05
    VELOCITY_THRESH = 0.1
    if theta > THETA_THRESH or  omega > OMEGA_THRESH:
        return 3
    elif theta < -THETA_THRESH or omega < -OMEGA_THRESH:
        return 1
    elif y < 1.0 and vy < -VELOCITY_THRESH:
        return 2
    elif vx < -H_VELOCITY_THRESH:
        return 3
    elif vx > H_VELOCITY_THRESH:
        return 1
    else:
...
def train(env, n_actions, args):
    ...
    final_exploration_count = 100000
